...
import HttpFetch from 'utils/http/HttpFetch';
...
const authLink = setContext((_, { headers }) => {
    return {
        headers: {
            ...headers,
            // FIXME: Authorization: affectliSso.getBearerToken(),
        },
    };
});
...
class GraphQlClient {
    ...
    handleError = (error: Object, options: Object) => {
        ...
        console.log(
            '[GraphQlClient] an error occured executing',
            Array.isArray(definitions) && definitions.map((def) => def.name && def.name.value).join(' '),
            ', variables:',
    ...
    query(options: Object) {
        isDev &&
            console.debug('[dev] graphql executing query', options.query.definitions.map((def) => def.name && def.name.value).join(' ')); // eslint-disable-line no-console
        return this.client
            .query(options)
    ...
    mutate(options: Object) {
        isDev &&
            // eslint-disable-next-line no-console
            console.debug(
                '[dev] graphql executing mutation',
                options.mutation.definitions.map((def) => def.name && def.name.value).join(' ')
            );
        return this.client
            .mutate(options)
            .then(this.handleResponse)
